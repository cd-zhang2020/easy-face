...
/**
 * 门禁 1:N 人脸识别 Activity（多对多）
 *
 * 完全原生实现：Camera2 预览 + 自定义 Canvas 覆盖层 + AI 管线
 * 与 FaceVerifyActivity 的区别：不针对单个 faceID，而是在启动时加载
 * 全部已注册人脸特征，对每帧抓拍做 1:N 检索（取最高余弦相似度），
 * 达标即判定为"开门/欢迎"，否则为"陌生人"。
 * 从 uni-app 通过 startActivityForResult 启动，结果通过 Intent extras 返回。
 *
 * 返回 Intent extras:
 *   "code": Int        - 结果码 (1=开门成功, 2=陌生人, 6=无录入人脸, 其他=失败/取消)
 *   "msg": String      - 结果描述
 *   "faceID": String   - 匹配到的用户 ID（陌生人/取消时为空）
 *   "similarity": Float- 最佳匹配相似度 (0~1)
 *   "matchCount": Int  - 连续匹配帧数
 */
class FaceAccessActivity : Activity() {

    companion object {
        private const val TAG = "FaceAccessAct"
        private const val REQUEST_CAMERA_PERMISSION = 2001

        // Intent extra keys
        const val RESULT_CODE = "code"
        const val RESULT_MSG = "msg"
        const val RESULT_FACE_ID = "faceID"
        const val RESULT_SIMILARITY = "similarity"
    ...
    // 1:N 检索数据源：启动时一次性加载全部已注册特征到内存，逐帧仅做点积
    private var registered: Map<String, FloatArray> = emptyMap()
    ...
    private fun initAndStart() {
        ...
        // 加载全部已注册人脸特征（1:N 数据源）
        registered = faceRepo!!.getAllFeatures()
        if (registered.isEmpty()) {
            finishWithError(6, "请先录入人脸特征")
            return
        }
        ...
        overlayView = OverlayView(this, registered.size).apply {
            layoutParams = FrameLayout.LayoutParams(
    ...
    private fun openCamera() {
        ...
        bgThread = HandlerThread("FaceAccessCam").apply {
     ...
     * 然后在竖屏图像上检测→裁剪→提取特征，最后对全部已注册特征做 1:N 检索。
     */
    private fun processYuvFrame(image: Image) {
        ...
        try {
            ...
            // 步骤5：1:N 检索 —— 遍历全部已注册特征取最大余弦相似度
            t0 = SystemClock.elapsedRealtime()
            var bestID = ""
            var bestScore = -1f
            for ((fid, stored) in registered) {
                val s = faceRepo!!.cosineSimilarity(embedding, stored)
                if (s > bestScore) {
                    bestScore = s
                    bestID = fid
                }
            }
            Log.d(TAG, "1:N recognize 耗时: ${SystemClock.elapsedRealtime() - t0}ms, best=$bestID score=$bestScore")
            faceBitmap.recycle()
            ...
            runOnUiThread {
                if (bestScore >= threshold && bestID.isNotEmpty()) {
                    matchCount++
                    failCount = 0
                    overlayView?.updateMatch(matchCount, failCount, bestID, bestScore)
                    if (matchCount >= MATCH_MIN) {
                        onAccessGranted(bestID, bestScore)
                    }
                } else {
                    failCount++
                    matchCount = maxOf(0, matchCount - 1)
                    overlayView?.updateMatch(matchCount, failCount, "", bestScore)
    ...
    private fun startVerify() {
        ...
        timeoutRunnable = Runnable {
            if (isVerifying) {
                isVerifying = false
                stopCamera()
                finishWithResult(9010004, "识别超时，请重试", matchCount, "", -1f)
            }
        }
    ...
    /**
     * 开门成功：显示欢迎动画并回传 code=1
     */
    private fun onAccessGranted(faceID: String, score: Float) {
        if (!isVerifying) return
        isVerifying = false
        cancelTimeout()
        stopCamera()
        runOnUiThread {
            overlayView?.showGranted(faceID)
            Toast.makeText(this, "开门成功，欢迎 $faceID", Toast.LENGTH_SHORT).show()
            Handler(Looper.getMainLooper()).postDelayed({
                finishWithResult(1, "开门成功，欢迎 $faceID", matchCount, faceID, score)
            }, 900)
        }
    }

    /**
     * 陌生人：连续失败达上限，回传 code=2
     */
    private fun onAccessDenied() {
        ...
        stopCamera()
        runOnUiThread {
            finishWithResult(2, "未匹配到已注册人员", matchCount, "", -1f)
        }
    }
    ...
    private fun goBack() {
        ...
        cancelTimeout()
        stopCamera()
        finishWithResult(0, "用户取消", matchCount, "", -1f)
    }
    ...
    private fun finishWithError(code: Int, msg: String) {
        ...
        val intent = Intent().apply {
            ...
            putExtra(RESULT_FACE_ID, "")
            putExtra(RESULT_SIMILARITY, -1.0)
    ...
    private fun finishWithResult(code: Int, msg: String, matchCount: Int, faceID: String, similarity: Float) {
        destroyAll()
        val intent = Intent().apply {
            putExtra(RESULT_CODE, code)
            putExtra(RESULT_MSG, msg)
            putExtra(RESULT_FACE_ID, faceID)
            putExtra(RESULT_SIMILARITY, similarity.toDouble())
    ...
    inner class OverlayView(context: Context, private val totalFaces: Int) : View(context) {
        ...
        private var scanAnimRunnable = object : Runnable {
            override fun run() {
                if (isVerifyingLocal) {
                    scanAnimPhase += 0.05f
        ...
        // 1:N 实时状态
        private var matchedName = ""
        private var matchedScore = -1f
        private var grantedName = ""
        private var successAnimPhase = 0f
        private var successAnimRunnable = object : Runnable {
            override fun run() {
                if (grantedName.isNotEmpty()) {
                    successAnimPhase += 0.06f
                    if (successAnimPhase > Math.PI * 2) successAnimPhase = 0f
                    invalidate()
                    postDelayed(this, 16)
                }
            }
        ...
        private val scanPaint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
            color = Color.argb(153, 16, 185, 129)
            style = Paint.Style.FILL
        }
        ...
        private val grantedTitlePaint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
            color = Color.rgb(16, 185, 129)
            textSize = 34f.sp
            isFakeBoldText = true
            textAlign = Paint.Align.CENTER
        ...
        private val grantedNamePaint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
            color = Color.WHITE
            textSize = 20f.sp
            isFakeBoldText = true
            textAlign = Paint.Align.CENTER
        ...
        private val successRingPaint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
            color = Color.argb(120, 16, 185, 129)
            style = Paint.Style.STROKE
            strokeWidth = 6f.dp
        }
        private val btnGradient = GradientDrawable(
            GradientDrawable.Orientation.LEFT_RIGHT,
            intArrayOf(Color.rgb(99, 102, 241), Color.rgb(124, 58, 237))
        )
        private val btnDisabledGradient = GradientDrawable(
            GradientDrawable.Orientation.LEFT_RIGHT,
            intArrayOf(
                Color.argb(128, 99, 102, 241),
                Color.argb(128, 124, 58, 237)
            )
        )
        ...
        fun stopAnimation() {
            ...
            grantedName = ""
            removeCallbacks(scanAnimRunnable)
            removeCallbacks(successAnimRunnable)
        }

        fun updateMatch(matched: Int, failed: Int, name: String, score: Float) {
            matchCount = matched
            failCount = failed
            matchedName = name
            matchedScore = score
        ...
        fun showGranted(name: String) {
            isVerifyingLocal = false
            grantedName = name
            successAnimPhase = 0f
            removeCallbacks(scanAnimRunnable)
            post(successAnimRunnable)
            invalidate()
        }
        ...
        override fun onTouchEvent(event: MotionEvent): Boolean {
            if (event.action == MotionEvent.ACTION_UP) {
                val x = event.x
                val y = event.y
                if (btnPrimaryRect.contains(x, y) && !isVerifyingLocal && grantedName.isEmpty()) {
                    onStartClick?.invoke()
                    return true
                }
        ...
        override fun onDraw(canvas: Canvas) {
            ...
            // 进门成功时整框变绿发光
            val framePaint = if (grantedName.isNotEmpty()) {
            ...
            // Corners
            canvas.drawLine(frameLeft, frameTop + cornerLen, frameLeft, frameTop + cornerStroke / 2, framePaint)
            canvas.drawLine(frameLeft, frameTop + cornerStroke / 2, frameLeft + cornerLen, frameTop + cornerStroke / 2, framePaint)
            canvas.drawLine(frameRight - cornerLen, frameTop + cornerStroke / 2, frameRight, frameTop + cornerStroke / 2, framePaint)
            canvas.drawLine(frameRight, frameTop + cornerStroke / 2, frameRight, frameTop + cornerLen, framePaint)
            canvas.drawLine(frameLeft, frameBottom - cornerLen, frameLeft, frameBottom - cornerStroke / 2, framePaint)
            canvas.drawLine(frameLeft, frameBottom - cornerStroke / 2, frameLeft + cornerLen, frameBottom - cornerStroke / 2, framePaint)
            canvas.drawLine(frameRight - cornerLen, frameBottom - cornerStroke / 2, frameRight, frameBottom - cornerStroke / 2, framePaint)
            canvas.drawLine(frameRight, frameBottom - cornerStroke / 2, frameRight, frameBottom - cornerLen, framePaint)

            // ── 进门成功动画（优先绘制，覆盖于最上层）──
            if (grantedName.isNotEmpty()) {
                drawGranted(canvas, w, h, frameLeft, frameTop, frameRight, frameBottom)
                return
            }

            // ── Scan line ──
            if (isVerifyingLocal) {
                val scanProgress = (Math.sin(scanAnimPhase.toDouble()) + 1.0) / 2.0
            ...
            // ── Title ──
            canvas.drawText("智能门禁核验", w / 2f, h * 0.05f, titlePaint)
            val statusText = when {
                failCount >= MAX_FAILS -> "未匹配到已注册人员"
                isVerifyingLocal -> "正在进行 1:N 人脸核验..."
                else -> "请将脸部对准摄像头 (库内 $totalFaces 人)"
            }
            canvas.drawText(statusText, w / 2f, h * 0.05f + 24f.dp, subtitlePaint)

            // ── 实时匹配提示 ──
            if (isVerifyingLocal && matchedName.isNotEmpty() && matchedScore >= 0) {
                val pct = String.format("%.1f", matchedScore * 100)
                canvas.drawText("识别中: $matchedName  $pct%", w / 2f, frameBottom + 18f.dp, matchPaint)
            }
            ...
            val progressText = if (isVerifyingLocal) "请正对摄像头，保持面部清晰" else "点击按钮开始门禁核验"
            ...
            val gradient = if (isVerifyingLocal) btnDisabledGradient else btnGradient
            gradient.setBounds(btnStartX.toInt(), primaryBtnY.toInt(), (btnStartX + btnWidth).toInt(), (primaryBtnY + btnHeight).toInt())
            gradient.cornerRadius = 12f.dp
            gradient.draw(canvas)
            val btnText = if (isVerifyingLocal) "核验中..." else "开始门禁识别"
            if (isVerifyingLocal) btnTextPaint.alpha = 128 else btnTextPaint.alpha = 255
            canvas.drawText(btnText, w / 2f, primaryBtnY + btnHeight / 2f - (btnTextPaint.descent() + btnTextPaint.ascent()) / 2f, btnTextPaint)
            ...
            backGradient.setBounds(btnStartX.toInt(), backBtnY.toInt(), (btnStartX + btnWidth).toInt(), (backBtnY + btnHeight).toInt())
            backGradient.draw(canvas)
            canvas.drawText("返回", w / 2f, backBtnY + btnHeight / 2f - (btnBackTextPaint.descent() + btnBackTextPaint.ascent()) / 2f, btnBackTextPaint)

            if (failCount >= MAX_FAILS) {
                val warnY = backBtnY + btnHeight + 20f.dp
                canvas.drawText("未匹配到已注册人员，请使用账号密码", w / 2f, warnY, subtitlePaint)
            }
        }

        /**
         * 绘制进门成功动画：整屏绿色蒙版 + 扩散光环 + 大号 ✓ 与欢迎语
         */
        private fun drawGranted(canvas: Canvas, w: Float, h: Float, frameLeft: Float, frameTop: Float, frameRight: Float, frameBottom: Float) {
            ...
            canvas.drawRect(0f, 0f, w, h, Paint(Paint.ANTI_ALIAS_FLAG).apply {
                color = Color.argb(90, 16, 185, 129)
                style = Paint.Style.FILL
            })

            // 扩散光环
            val cx = w / 2f
            val cy = frameTop + frameHeight / 2f
            val ringBaseR = minOf(frameWidth, frameHeight) * 0.42f
            val ringPulse = (Math.sin(successAnimPhase.toDouble()) + 1.0) / 2.0
            val ringR = ringBaseR * (0.9f + ringPulse.toFloat() * 0.18f)
            successRingPaint.alpha = (120 * (1 - ringPulse)).toInt().coerceAtLeast(20)
            canvas.drawCircle(cx, cy, ringR, successRingPaint)
            canvas.drawCircle(cx, cy, ringR * 0.78f, successRingPaint)
            ...
            val cs = ringBaseR * 0.34f
            val cx0 = cx - cs * 0.5f
            val cy0 = cy + cs * 0.05f
            canvas.drawLine(cx0, cy0, cx0 + cs * 0.42f, cy0 + cs * 0.5f, checkPaint)
            canvas.drawLine(cx0 + cs * 0.42f, cy0 + cs * 0.5f, cx0 + cs * 1.4f, cy0 - cs * 0.7f, checkPaint)

            // 标题 + 欢迎语
            val bannerY = frameBottom + 50f.dp
            canvas.drawText("✓ 开门成功", w / 2f, bannerY, grantedTitlePaint)
            canvas.drawText("欢迎 $grantedName 进门", w / 2f, bannerY + 34f.dp, grantedNamePaint)
        }
    }
